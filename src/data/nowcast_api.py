...
import datetime
import numpy as np
# Use the following for testing nowcast(lat=35.909375, lon=-100.265042, radius=200, time_utc='2019-06-15 05:05:00', model_type='gan',closest_radius=True,threshold_time_minutes= 60, force_refresh=False)
...
def nowcastBatch(params_list):
    # Same code as nowcast, but in loop for all inputs
    filename_final = []
    data_final = []
    data_index = []
    display_path_final = []
    ...
    for param in params_list:
        ...
        try:
            # Filter to get data as BaseModel parameters
            ...
            if data_final == []:
               ...
               # Stack all input params as one and then run the model, split later into individual outputs 
    ...
    for index,filename in zip(data_index,filename_final):
        ...
        # Split output GIF for all input sets
